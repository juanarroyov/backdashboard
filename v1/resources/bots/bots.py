...
from v1.models.bots.bot_config import BotsModel
...
class Bots(Resource):
    ...
    def post(self, id_bot=None, fecha_creacion=None):
        cliente = 'cliente_test' #Cliente debe salir de los datos de autentificacion
        ...
        valid, error = mongo_bots.post(cliente=cliente, fecha_creacion=fecha_created, fecha_modificacion=fecha_mod, recurso=recurso)
        if not valid:
            return {"message": error}, 400
    ...
    def get(self, id_bot):
        ...
        if id_bot:
            ...
            #retornar valores
            recurso, valid, error = mongo_bots.get(cliente, id_bot)
            if not valid:
                return {"message": error}, 400
        ...
        #! Intentar usar paginacion.
            ...
            results = mongo_bots.get(cliente, None, url, start, limit)        
    ...
    def put(self, id_bot):
        ...
        exist_idconfig = mongo_bots.exist_id(cliente, id_bot)
        if not exist_idconfig:
            return {"message": "No existe el id del bot que desea actualizar."}, 400
        #id existe
        #encontrar fecha de creacion
        fecha = mongo_bots.get_fecha_creacion(cliente, id_bot)
        #hash de id_bot
        id_bot_tmp = hashlib.sha1(id_bot.encode()).hexdigest()
        #remplazar id_bot en la base por el hash
        mongo_bots.rename_id(cliente, id_bot, id_bot_tmp)
        recurso = self.post(id_bot=id_bot, fecha_creacion=fecha)
        ...
        self.delete(id_bot_tmp)
        return recurso

        
    def delete(self, id_bot):
        cliente = 'cliente_test'
        #Si existe un id la funcion elimina la configuracion de lista del id 
        if id_bot:
            #validar que existe id_bot en la base de datos
            #eliminar recurso
            valid, error = mongo_bots.delete(cliente, id_bot)
            if not valid:
                return {"message": error}, 400
            return {"message": 'El bot "'+id_bot+'" fue eliminado.'}, 200
        #Si no existe id la funcion retorna todas las configuraciones de lista
        else:
            valid, error = mongo_bots.delete(cliente, id_bot)
            if not valid:
